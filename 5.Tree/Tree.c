...
RbTreeNode* RBT_Insert(RbTreeNode* root, DataType data) {
    if(NULL == root) {
        ...
        if(NULL == p) {
            perror("RBT_Insert fail as malloc error");
            return NULL;
        }
        p->data = data;
        p->parent = NULL;
    ...
    if(data < root->data && root->left != nilptr) {
        root = RBT_Insert(root->left, data);
    } else if(data > root->data && root->right != nilptr){
        root = RBT_Insert(root->right, data);
    } else {
        RbTreeNode* p = (RbTreeNode* )malloc(sizeof(RbTreeNode));
        if(NULL == p) {
            perror("RBT_Insert fail as malloc error");
        ...
        if(BLACK == root->color) {
            ...
            RbTreeNode* uncle = root->parent->left == nilptr ? root->parent->right : root->parent->left;
            RbTreeNode* parent = root;
            RbTreeNode* grandpa = parent->parent;
            if(BLACK == uncle->color) {
                if(parent == grandpa->left) {
                    if(p = parent->right) {
                        parent = RBT_LeftDown(parent);
                    }
                    grandpa = RBT_RightDown(grandpa);
                } else {
                    if(p = parent->left) {
                        parent = RBT_RightDown(parent);
                    }
                    grandpa = RBT_LeftDown(grandpa);
                }
                return root;
            }
            while(RED == uncle->color && RED == parent->color) {
                uncle->color = BLACK;
                parent->color = BLACK;
                ...
                uncle = parent->left == nilptr? parent->right : parent->left;
            }
        }
    }
}
